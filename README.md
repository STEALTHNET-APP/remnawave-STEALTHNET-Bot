...
|--------|-----|-----|------|------------|
| **Минимальная** | 1 vCPU | 2 GB | 20 GB | Тест, демо, до ~50 активных пользователей |
...
|--------|-----------|------------|
| **backend** | Node.js, Express, Prisma, PostgreSQL | REST API: авторизация, клиенты, тарифы, платежи, рефералы, промо, аналитика, бэкапы |
...
- **Platega.io** — приём платежей (карты, СБП, криптовалюта и др.); callback URL копируется в админке
...
- **Описание тарифов** — в редактировании тарифов можно указывать описания и преимущества подписок
...
- **Ручные рассылки** — отправка сообщений с изображениями/файлами клиентам в Telegram или на Email
- **Автоматические рассылки** — настраиваемые правила: после регистрации, неактивность, без платежа
...
- **Менеджеры** — создание менеджеров с многоуровневой системой доступа. 
...
| **Промо-ссылки** | Создание и управление промо-ссылками |
| **Промокоды** | Создание скидочных и бесплатных промокодов |
| **Аналитика** | Графики выручки, клиентов, рефералов, конверсии |
| **Маркетинг** | UTM-метки, полезные ссылки для рекламы, счетчики: Яндекс Метрика, Google Analytics 4 |
| **Отчёт продаж** | Детализация продаж с фильтрами |
| **Рассылка** | Отправка сообщения клиентам с изображениями/файлами в Telegram и/или на email |
| **Авто-рассылка** | Cron-рассылки после регистрации, неактивность, без платежа |
| **Бэкапы** | Создание и восстановление резервной копии базы данных |
...
| **Менеджеры** | Создавайте менеджеров и назначайте им доступ только к нужным разделам админки |
...
# Перезапуск API, бота и фронтенда
docker compose restart api bot frontend

# Запуск (без встроенного nginx)
docker compose up -d
# Полная остановка (без встроенного nginx)
docker compose down
...
docker compose down
docker compose up -d --build frontend api bot

# Если не помогло, то вручную:
docker compose build api bot frontend
docker compose up -d
```

### Корректное обновление из Git (git pull)

Файл **`nginx/nginx.conf`** генерируется скриптом **`install.sh`** каждый раз после **`git pull`** и не должен отслеживаться Git.
В .gitignore уже добавлено: **`nginx/nginx.conf`**.

- Если по какой то причине Git всё ещё его обновляет при pull, ломая конфигурацию, то выполните один раз:  
  `git rm --cached nginx/nginx.conf && git commit -m "Stop tracking nginx.conf"`
- Если есть локальные изменения, перед обновлением нужно их сохранить:  
  `git stash && git pull && git stash pop` — спрячьте изменения, обновите проект и верните обратно.  
  *Если появятся конфликты — Git подскажет, какие файлы нужно исправить.* 
- Если локальные изменения не нужны, но файлы модифицированы:  
  `git fetch origin && git reset --hard origin/main && git clean -fd` — эта команда удалит все локальные изменения.
...
| `INIT_ADMIN_EMAIL` | да | Email администратора |
| `INIT_ADMIN_PASSWORD` | да | Пароль администратора |
